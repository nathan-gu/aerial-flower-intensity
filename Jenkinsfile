...
node {
	stage('Checkout') {
		...
		withAWS(credentials: "${env.ENVIRONMENT}-user-password-form", region: 'eu-central-1') {
			stage('Private wheels download') {
				s3Download(
					file: 'wheels/',
					bucket: "cloverfield-modules-provisioning-${env.ENVIRONMENT}",
					path: 'wheels/'
					)
			}
		}
